...
export class AgentLogger {
        ...
        values = Object.assign(
            {
                ...
                _sourceHost: this.agent.machineId ? this.agent.machineId : os.hostname(),
    ...
    async LogWarning(msg: string, values: any) {
        await this.Log(
            Object.assign({ msg: msg, TeamId: this.agent.teamId, AgentId: this.instanceId }, values),
            LogLevel.WARNING
        );
    }
                ...
                const files_extended = await files
                    .filter((fileName) => {
                        ...
                        return (
                            ...
                            (path.extname(filePath) == '.log' || path.extname(filePath) == '.gz') &&
    ...
    async UploadLogFile(filePath: string, fileSize: number) {
        ...
        try {
            ...
            if (fileExtension == '.log') {
                if (fileSize > this.maxLogFileUploadSize) fs.truncateSync(filePath, this.maxLogFileUploadSize);
