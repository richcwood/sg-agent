...
export class RabbitMQAdmin {
    ...
    async getQueues(matchStr: string) {
        return new Promise((resolve, reject) => {
            this.instance
                .get(`queues/${this.vhost}`)
                .then((response) => {
                    if (!response.data) resolve('');
                    const queues: string[] = [];
                    for (let i = 0; i < response.data.length; i++) {
                        if (response.data[i].name.match(matchStr)) queues.push(response.data[i].name);
                    }
                    resolve(queues);
                })
                .catch((err) => {
    ...
    async getExchanges(matchStr: string) {
        return new Promise((resolve, reject) => {
            this.instance
                .get(`exchanges/${this.vhost}`)
                .then((response) => {
                    if (!response.data) resolve('');
                    const exchanges: string[] = [];
                    for (let i = 0; i < response.data.length; i++) {
                        if (response.data[i].name.match(matchStr)) exchanges.push(response.data[i].name);
                    }
                    resolve(exchanges);
                })
                .catch((err) => {
    ...
    async purgeQueue(queue: string) {
        return new Promise<any>((resolve, reject) => {
            this.instance
                .delete(`queues/${this.vhost}/${queue}/contents`)
                .then((response) => {
    ...
    async deleteQueue(queue: string) {
        return new Promise<any>((resolve, reject) => {
            this.instance
                .delete(`queues/${this.vhost}/${queue}`)
                .then((response) => {
                    resolve(response.status);
                })
                .catch((err) => {
    ...
    async createQueue(queue: string, autoDelete: boolean, durable: boolean, expires = 0) {
        return new Promise((resolve, reject) => {
            const args = { 'auto-delete': autoDelete, durable: durable };
            if (expires > 0) Object.assign(args, { arguments: { 'x-expires': expires } });
            this.instance
                .put(`queues/${this.vhost}/${queue}`, args)
                .then((response) => {
                    resolve(response.status);
                })
                .catch((err) => {
    ...
    async createExchange(exchange: string, type: string, autoDelete: boolean, durable: boolean) {
        return new Promise((resolve, reject) => {
            this.instance
                .put(`exchanges/${this.vhost}/${exchange}`, {
                    type: type,
    ...
    async bindExchanges(source: string, destination: string, routingKey: string) {
        return new Promise((resolve, reject) => {
            this.instance
                .post(`bindings/${this.vhost}/e/${source}/e/${destination}`, {
                ...
                .catch((err) => {
                    reject(
                        new Error(
                            `Error binding exchange '${source}" to "${destination}" with route "${routingKey}': ${err}`
                        )
                    );
                });
        });
    ...
    async bindQueueToExchange(exchange: string, queue: string, routingKey: string) {
        return new Promise((resolve, reject) => {
            this.instance
                .post(`bindings/${this.vhost}/e/${exchange}/q/${queue}`, {
                    routing_key: routingKey,
                })
                .then((response) => {
                    resolve(response.status);
                ...
                .catch((err) => {
                    reject(
                        new Error(
                            `Error binding queue '${queue}" to exchange "${exchange}" with route "${routingKey}': ${err}`
                        )
                    );
                });
        });
    ...
    async deleteExchange(exchange: string) {
        return new Promise((resolve, reject) => {
            this.instance
                .delete(`exchanges/${this.vhost}/${exchange}`)
                .then((response) => {
                    resolve(response.status);
                })
                .catch((err) => {
    ...
    async createUser(userName: string, password: string, exchange: string) {
        return new Promise((resolve, reject) => {
            this.instance
                .put(`users/${userName}`, { password: password, tags: 'administrator' })
                .then((response) => {
                    this.instance
                        .put(`permissions/${this.vhost}/${userName}`, {
                            configure: '.*',
                            write: '.*',
                            read: '.*',
                        })
                        .then((response) => {
                            this.instance
                                .put(`topic-permissions/${this.vhost}/${userName}`, {
                                    exchange: exchange,
                                    write: '.*',
                                    read: '.*',
                                })
                                ...
                                .catch((err) => {
                                    reject(
                                        new Error(
                                            `Error setting '${userName}" permissions for exchange "${exchange}': ${err}`
                                        )
                                    );
                                });
                        })
        ...
        return new Promise((resolve, reject) => {
            this.instance
                ...
                .then((response) => {
                    ...
                    const users: string[] = [];
                    for (let i = 0; i < response.data.length; i++) {
                        if (response.data[i].name.match(matchStr)) users.push(response.data[i].name);
                    }
                    resolve(users);
                })
                .catch((err) => {
    ...
    async setPolicy(policyName: string, pattern: string, definition: any, applyTo = 'all', priority = 0) {
        return new Promise((resolve, reject) => {
            this.instance
                .put(`policies/${this.vhost}/${policyName}`, {
                    pattern: pattern,
