...
export class StompConnector {
  ...
  LogInfo(content: string, consumerTag: string, redelivered: boolean, destination: string, values: any) {
    this.logger.LogInfo(
      content,
      Object.assign(
        { ClientId: this.clientId, ConsumerTag: consumerTag, Redelivered: redelivered, Destination: destination },
        values
      )
    );
  }
                  ...
                  if (!noAck) {
                    if (this.activeMessages.indexOf(msgKey) > -1) {
                      try {
                        ...
                        this.LogError("Error occurred acking Stomp message: " + e.message, e.stack, {
